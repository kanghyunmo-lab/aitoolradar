        ...
        <form action="/search" method="GET">
          <div className="flex gap-2">
            <input
              ...
              autoFocus
              className="flex-1 rounded-lg border border-gray-300 px-4 py-3 text-base focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            <button
              type="submit"
        ...
        <>
          ...
          {results.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed border-gray-300 p-12 text-center">
              <p className="text-gray-500">No tools found for &ldquo;{query}&rdquo;</p>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {["Jasper", "ChatGPT", "Midjourney", "Copy.ai", "Zapier"].map((s) => (
              <Link
                key={s}
                href={`/search?q=${s}`}
                className="rounded-full border border-gray-200 px-4 py-1.5 text-sm text-gray-600 hover:border-blue-300 hover:text-blue-600"
              >
                {s}
              </Link>
            ))}
